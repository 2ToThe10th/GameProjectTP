...
* Построить башню в городе ```CreateTower {city_id}```
* Конец хода ```End```
...
* Перейти на соседнюю клетку боевым юнитом ```GoCombat {combat_unit_id} {where}```
* Перейти на соседнюю клетку рабочим ```GoWorker {worker_id} {where}```
* Перейти на соседнюю клетку колонистом ```GoColonist {colonist_id} {where}```
* Аттака юнитом ```Attack {combat_unit_id} {where}```
